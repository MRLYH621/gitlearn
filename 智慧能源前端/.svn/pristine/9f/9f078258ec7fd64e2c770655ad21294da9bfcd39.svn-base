                                        ...
                                        <el-dialog title="参数信息编辑" :visible.sync="dialogVisible" width="30%" :before-close="handleClose">
                                                ...
                                                <el-form :label-position="labelPosition" label-width="100px" :model="model">
                                                        ...
                                                        <el-form-item label="参数代码：">
                                                                <el-input v-model="model.Code" placeholder="请输入参数代码"></el-input>
                                                        </el-form-item>
...
export default {
        ...
        methods: {
                ...
                append(item,index){
                        ...
                        this.model.Name = item.D_Name;
                        this.model.Code = item.D_Code;
                        this.model.Value = item.D_Value;
                        this.model.Unit = item.D_Unit;
                        this.model.saveOrUpd = true;
                },
                dele(ID){
                        var oldthis = this;
                        var params = new URLSearchParams();
                        var url = this.$api+'Plat/DeleteDatum';
                        params.append('',ID);
                ...
                submitone(){
                        ...
                        if(this.model.saveOrUpd){
                                url = this.$api+'Plat/UpdateDatum';   
                                params.append('ID',this.model.ID);    
                        }else{
                                url = this.$api+'Plat/AddDatum';    
                        }                 
                        var oldThis = this;
                        if(this.model.Name ==''){
                                alert("请输入参数名称！");
                        ...
                        params.append('D_Name',this.model.Name);     
                        params.append('D_Code',this.model.Code);
                        params.append('D_Value',this.model.Value);
                        params.append('D_Unit',this.model.Unit);
                        this.$axios.post(url, params).then(function (response) {
                                if(response.data == -1){
                                        alert('保存失败！');
                ...
                getData(){
                        var url = this.$api+'Plat/GetDatum';
                        var oldThis = this;
                ...
                getDataByName(){
                        var url = this.$api+'Plat/GetDatum?Name='+this.Name;
                        var oldThis = this;
                        this.$axios.get(url).then(function (response) {
                                oldThis.totalData2=[];
                                oldThis.totalData2=response.data;
