...
namespace CoronaDataCenter.Business
{
    public class StationInofManager
    {
        ...
        {
            List<StationInfoModel> list = new List<StationInfoModel>();
            String sql = "SELECT * FROM dbo.Tb_Station_Info a INNER JOIN  dbo.Tb_SysEvaluation b ON a.sn=b.parm_002 ";
            DataTable dt = DataAccess.ExecuteGetDataTable(sql);
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    model.SysType = String.IsNullOrEmpty(dt.Rows[i]["StaSysType"].ToString()) ? 0 : Convert.ToInt16(String.IsNullOrEmpty(dt.Rows[i]["StaSysType"].ToString()));
                    model.SysMode = String.IsNullOrEmpty(dt.Rows[i]["StaSysMode"].ToString()) ? 0 : Convert.ToInt16(String.IsNullOrEmpty(dt.Rows[i]["StaSysMode"].ToString()));
                    model.EvalValue = String.IsNullOrEmpty(dt.Rows[i]["pt_parm_002"].ToString()) ? 0 : Convert.ToSingle(String.IsNullOrEmpty(dt.Rows[i]["pt_parm_002"].ToString()));
                    list.Add(model);
                }
            }
            return list;
        ...
        {
            List<StationInfoModel> list = new List<StationInfoModel>();
            String sql = "SELECT *  FROM  dbo.Tb_Station_Info WHERE StaSysType = " + SysType + " AND StaSysMode = " + SysMode;
            DataTable dt = DataAccess.ExecuteGetDataTable(sql);
        ...
        {
            List<StationInfoModel> list = new List<StationInfoModel>();
            String sql = "SELECT *  FROM  dbo.Tb_Station_Info WHERE StaSysType = " + SysType;
            DataTable dt = DataAccess.ExecuteGetDataTable(sql);
        ...
        {
            List<StationInfoModel> list = new List<StationInfoModel>();
            String sql = "SELECT *  FROM  dbo.Tb_Station_Info WHERE StaSysMode = " + SysMode;
            DataTable dt = DataAccess.ExecuteGetDataTable(sql);
        ...
        #region StationTypeModel

        public List<StationTypeModel> GetStationType()
        {
            List<StationTypeModel> list = new List<StationTypeModel>();
            String sql = "SELECT * FROM Tb_Station_Type ";
            DataTable dt = DataAccess.ExecuteGetDataTable(sql);

            if (dt != null && dt.Rows.Count > 0)
            {
                StationTypeModel model;
                for (int i = 0; i < dt.Rows.Count; i++)
                {
        ...
        {
            List<StationModeModel> list = new List<StationModeModel>();
            String sql = "SELECT * FROM Tb_Station_Mode ";
            DataTable dt = DataAccess.ExecuteGetDataTable(sql);

            if (dt != null && dt.Rows.Count > 0)
            {
                StationModeModel model;
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    model = new StationModeModel();
                    model.ID = Convert.ToInt32(dt.Rows[i]["ID"].ToString());
                    model.M_Name = dt.Rows[i]["M_Name"].ToString();
                    list.Add(model);
                }
            }
            return list;
